...
from .flow_sql import all_agents, state_transition
...
parser.add_argument('--template', type=str, default='v2', help='template version')
...
class ExperimentWrapper():
    def __init__(self, args):
        ...
        file_pre = f"{args.env}{added_str}{name_str}_t_{args.template}_s_{args.cache_seed}"

        log_file_name = file_pre + ".json"
        self.log_path = os.path.join(args.log_dir, log_file_name)
        summary_file_name = file_pre + "_summary.json"
    ...
    def run_expr(self):
        try:
            ...
            for idx in tqdm(range(len(self.env.data_loader)), disable=self.args.verbose):
                ...
                manager = GroupChatManager(groupchat=sql_chat, llm_config=None)


                # state 0: Init: execute[SHOW TABLES]
                ...
                observation, reward, done, _ = self.env.step("SHOW TABLES")
                intercode_agent.num_rounds = 1
                intercode_agent.turn_history = {
                    "thoughts": [init_thought],
                    "actions": ["execute[SHOW TABLES]"],
                ...
                message = f"Question: {self.env.query}\n{init_thought}\nAction: execute[SHOW TABLES]\nObservation: {observation}"
                init_agent.initiate_chat(manager, message=message)
